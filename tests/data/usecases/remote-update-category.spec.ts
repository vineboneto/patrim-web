...
import { mockCategoryModel, mockUpdateCategoryParams } from '@/tests/domain/mocks'

import faker from 'faker'

type SutTypes = {
  sut: RemoteUpdateCategory
  httpClientSpy: HttpClientSpy
}

const makeSut = (url = faker.internet.url()): SutTypes => {
  const httpClientSpy = new HttpClientSpy()
  const sut = new RemoteUpdateCategory(httpClientSpy, url)
  return {
    sut,
    httpClientSpy
  }
